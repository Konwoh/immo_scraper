...
from crawler.factory import create_factory
from crawler.base_crawler import crawler_logger
from datetime import datetime, timezone
import time

class CrawlerService:
    def process(self, param: SearchParams, currentPage: int):
    ...
    def start_crawler(self, search_params_id: int):
        session = None
        try:
            currentPage = 1
            ...
            continue_flag = True
            while continue_flag:
                result = retry(lambda: self.process(param, currentPage))
                if result is None:
            ...
            crawler_logger.exception("Error while processing crawler job: ", str(e))
            raise